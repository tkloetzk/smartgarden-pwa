...
export default defineConfig({
  ...
  test: {
    ...
    exclude: ["**/node_modules/**", "**/dist/**", "**/cypress/**", "**/.{idea,git,cache,output,temp}/**", "**/{karma,rollup,webpack,vite,vitest,jest,ava,babel,nyc,cypress,tsup,build}.config.*", "**/e2e/**", "**/test-utils/**", "**/*.stories.{js,jsx,ts,tsx}"],
    // Coverage configuration
    coverage: {
      provider: "v8",
      reporter: ["text", "json", "html"],
      include: ["src/**/*.{ts,tsx}"],
      exclude: ["src/**/*.d.ts", "src/main.tsx", "src/vite-env.d.ts", "src/vitest.setup.ts", "src/**/__tests__/**", "src/**/*.{test,spec}.{ts,tsx}", "src/test/**"],
      thresholds: {
        global: {
          branches: 80,
          functions: 80,
    ...
    clearMocks: true,
    restoreMocks: true,
    // Temporarily disable Storybook testing integration
    // projects: [{
    //   extends: false,
    //   plugins: [
    //     storybookTest({
