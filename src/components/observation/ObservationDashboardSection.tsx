...
import ObservationTaskCard from "@/components/observation/ObservationTaskCard";
...
interface ObservationDashboardSectionProps {
...
const ObservationDashboardSection: React.FC<ObservationDashboardSectionProps> = ({
  tasks,
  onTaskComplete,
  onTaskBypass,
  onTaskLogActivity,
  ...
  const relevantTasks = tasks.filter((task) => {
    ...
    // For future tasks, show up to 7 days ahead (observations are less frequent)
  ...
  if (relevantTasks.length === 0) {
    console.log("ObservationDashboardSection: No relevant tasks, not rendering");
    return null;
  }

  console.log("ObservationDashboardSection: Rendering with", relevantTasks.length, "tasks");
  ...
  const handleTaskCardClick = (task: ObservationTask) => {
    // For grouped tasks, use the first plant ID, or fall back to single plantId
    const plantId = (task as any).plantIds?.[0] || task.plantId;
    navigate(`/log-care/${plantId}?type=observe&focus=${encodeURIComponent(task.details.focus || 'general')}`);
  };
  ...
  return (
    <Card className="mb-6" data-testid="observation-dashboard-section">
      <CardHeader
...
export default ObservationDashboardSection;